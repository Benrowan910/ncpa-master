...
class ServiceNode(listener.nodes.LazyNode):
    def get_service_method(self, *args, **kwargs):
        ...
        if uname == "Windows":
            ...
            try:
                process = subprocess.Popen(
                    ["which", "initctl"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
                process.wait()
                if process.returncode == 0:
                    is_upstart = True
    ...
    def get_services_via_initd(self, *args, **kwargs):
        ...
        try:
            possible_services = filter(
                lambda x: os.stat("/etc/init.d/" + x)[ST_MODE]
    ...
    def get_services_via_svcs(self, *args, **kwargs):
        ...
        # The first line is the header
        header = status.readline().decode("utf-8").strip()

        for line_bytes in status.readlines():
    ...
    def make_stdout(returncode, stdout_builder):
        ...
        prioritized_stdout = sorted(
            stdout_builder, key=lambda x: x["priority"], reverse=True
        )
    ...
    def run_check(self, *args, **kwargs):
        ...
        if services:
            ...
            if filtered_services:
                for service in filtered_services:
                    stdout_builder.append(
                        {"info": "%s could not be found" % service, "priority": 0}
                    )
                returncode = 3
            ...
            returncode = 3
            stdout = "UNKNOWN: No services found for service names: %s" % ", ".join(
                filtered_services
            )
